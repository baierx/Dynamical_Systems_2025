...
# ## Rate and Change of Rate as a function of Substrate concentration
...
# ## Interactive Graph: slider for Hill coefficient
...
# ## Optional: Comparison of Hill approximation with full mechanism
...
def full_cooperative_kinetics(S, k_cat, E_total, Kd_values):
    ...
    Kd_values: dissociation constants for each binding step [Kd₁, Kd₂, ..., Kdₙ]
    """
    n = len(Kd_values)
    ...
    # Fraction of enzyme in fully bound state ESⁿ
    f_ESn = Z_terms[-1] / Z

    # Reaction rate = k_cat * [ESⁿ]
...
def get_f_ESn(S, k_cat, E_total, Kd_values):
    """Helper function to get just the fraction of ESⁿ"""
    n = len(Kd_values)
    Z_terms = [1]
    for i in range(1, n + 1):
        binding_term = 1
...
fig = make_subplots(
    ...
    subplot_titles=(
        ...
        'Fraction of Enzyme in ESⁿ State',
...
for i, n in enumerate(n_values):
    ...
    full_rates_equal = full_cooperative_kinetics_equal_Kd(S_range, k_cat, E_total, Kd_equal, n)
    ...
    # Plot fraction of ESⁿ
    fig.add_trace(
        go.Scatter(x=S_range, y=f_ESn_equal, 
                  name=f'f_ESⁿ n={n}', line=dict(color=colors[i]),
...
Kd_patterns = {
    ...
    'Decreasing Kd': [10.0, 2.0, 0.5, 0.1],  # Binding gets easier (positive cooperativity)
...
for j, (pattern_name, Kd_list) in enumerate(Kd_patterns.items()):
    full_rates_pattern = full_cooperative_kinetics(S_range, k_cat, E_total, Kd_list)
...
full_rates_coop = full_cooperative_kinetics(S_range, k_cat, E_total, Kd_positive_coop)
...
# Update axes
fig.update_xaxes(title_text="Substrate Concentration [S]", type="log", row=1, col=1)
fig.update_yaxes(title_text="Reaction Rate", row=1, col=1)
fig.update_xaxes(title_text="Substrate Concentration [S]", type="log", row=1, col=2)
fig.update_yaxes(title_text="Fraction f_ESⁿ", row=1, col=2)
fig.update_xaxes(title_text="Substrate Concentration [S]", type="log", row=2, col=1)
fig.update_yaxes(title_text="Reaction Rate", row=2, col=1)
fig.update_xaxes(title_text="Substrate Concentration [S]", type="log", row=2, col=2)
...
print(f"{'S':>6} {'Hill':>8} {'Equal Kd':>8} {'Pos Coop':>8} {'f_ES2_eq':>8} {'f_ES2_coop':>8}")
print("-" * 60)
...
for s in S_sample:
    ...
    print(f"{s:6.1f} {hill_rate:8.4f} {full_equal:8.4f} {full_coop:8.4f} {f_ES2_eq:8.4f} {f_ES2_coop:8.4f}")
...
print("4. Hill coefficient n is a measure of apparent cooperativity, not necessarily number of sites")
