...
class SimpleCICR2D:
    """
    Simplified 2D CICR model with only Ca (X) and IP3 (Y)
    """
    ...
    def set_stimulation(self, amplitude=1.0, frequency=0.1, radius=5, phase_shift=0.0, center=None):
        """
...
def run_simulation(stim_type="target", amplitude=1.5, frequency=0.1, radius=5, phase_shift=0.0, 
                   L=128, D_x=0.5, D_y=0.0, time_steps=5000):
    """
    ...
    - phase_shift: phase shift for spirals (ignored for target patterns)
    """
...
def parameter_sweep():
    """Test different parameters to find optimal spiral conditions"""
    test_cases = [
        # (stim_type, amplitude, frequency, radius, phase_shift, description)
        ("target", 2.0, 0.1, 3, 0.0, "Standard target patterns"),
        ("spiral", 1.5, 0.08, 8, 0.5*np.pi, "Moderate spirals"),
        ("spiral", 2.0, 0.05, 12, 1.0*np.pi, "Strong spirals"),
        ("spiral", 1.0, 0.12, 6, 0.25*np.pi, "Weak spirals"),
    ]
    
    for stim_type, amp, freq, rad, phase, desc in test_cases:
        print(f"\n{'='*50}")
        print(f"Testing: {desc}")
        print(f"Parameters: amp={amp}, freq={freq}, radius={rad}, phase_shift={phase/np.pi:.2f}π")
        print(f"{'='*50}")
        ...
        model = run_simulation(
            ...
            radius=rad,
            phase_shift=phase,
            time_steps=2000  # Shorter for testing
        )


# Example usage
if __name__ == "__main__":
    print("Clean 2D CICR Model - Unified Stimulation Control")
    
    # Simple usage examples:
    
    # 1. Target patterns (symmetric)
    # model = run_simulation(stim_type="target", amplitude=2.0, frequency=0.1, radius=3)
    
    # 2. Spiral patterns (asymmetric)
    model = run_simulation(stim_type="spiral", amplitude=1.0, frequency=0.12, 
                          radius=3, phase_shift=0.0*np.pi)
    # model = run_simulation(stim_type="spiral", amplitude=1.0, frequency=0.12, 
    #                       radius=3, phase_shift=0.4*np.pi) # Spiral
    # 3. Parameter sweep to find optimal conditions
    # parameter_sweep()
